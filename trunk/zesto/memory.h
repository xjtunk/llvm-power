...
/* SimpleScalar(TM) Tool Suite Copyright (C) 1994-2002 by Todd M.
 * Austin, Ph.D. and SimpleScalar, LLC.  All Rights Reserved. 
 * 
 * THIS IS A LEGAL DOCUMENT, BY USING SIMPLESCALAR, YOU ARE
 * AGREEING TO THESE TERMS AND CONDITIONS.
 * 
 * No portion of this work may be used by any commercial entity, or
 * for any commercial purpose, without the prior, written
 * permission of SimpleScalar, LLC (info@simplescalar.com).
 ...
 * Arbor, MI 48105.
 * 
 * Copyright (C) 1994-2002 by Todd M. Austin, Ph.D. and
 * SimpleScalar, LLC.
 */
...
#define PAGE_TABLE_ADDR(C,A) ((((A)>>PAGE_SHIFT) + 0x00080000) & 0x03ffffff)
...
struct mem_t {
  ...
  //int num_core_pages;
  //int max_core_pages;
  //int num_locked_pages; /* num mmaped pages that we don't write to backing file */
  struct mem_pte_t * recency_lru; /* oldest (LRU) */
  struct mem_pte_t * recency_mru; /* youngest (MRU) */
  //struct mem_page_link_t * page_free_list;
  //struct mem_page_link_t * link_free_list;
...
{ int idx;                                                  \
  (ACC) = (dram_access_t*) calloc(1,sizeof(dram_access_t));   \
...
#define MEM_PAGE(MEM, ADDR)            \
  (/* first attempt to hit in first entry, otherwise call xlation fn */  \
   ((MEM)->ptab[MEM_PTAB_SET(ADDR)]          \
    ...
    && (MEM)->ptab[MEM_PTAB_SET(ADDR)]->page != NULL)  \
...
// replace NULL below with valid byte_t ptr
