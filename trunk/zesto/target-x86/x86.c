/*
 * x86.c - x86 ISA definition routines
 *
...
#include "memory.h"
#include "sim.h"
#include "target-x86/x86flow.def"
...
#error SimpleScalar/x86 only builds on little-endian machines...
#endif
#endif
...
void
md_fetch_inst(md_inst_t *inst, struct mem_t *mem, md_addr_t pc)
{
  int i;
...
inline void
md_use_fetched_inst(md_inst_t *inst, byte_t *mem, md_addr_t pc)
{
  int i;
  ...
  const int
md_cc_eval(int cond, dword_t aflags, bool * bogus)
{
  int res;
...
/* decode an instruction */
  enum md_opcode
md_decode(byte_t mode, struct md_inst_t *inst, int set_nop)
{
...
md_print_ifmt(const char *fmt,
    struct Mop_t * Mop,		/* instruction to disassemble */
    FILE *stream)
{
  const char *s = fmt;
...
md_xor_regs(struct regs_t *regs)
{
  ...
  /* skip ZERO register */
  for (i=0; i < 8; i++)
    checksum ^= regs->regs_R.dw[i];
#if 0
  for (i=0; i < 8; i++)
  {
    checksum ^= E2Qw(regs->regs_F.e[i]);
    checksum ^= E2qW(regs->regs_F.e[i]);
  }
#endif

  checksum ^= regs->regs_C.aflags;
  checksum ^= regs->regs_C.fsw;
  checksum ^= regs->regs_PC;

  return (dword_t)((checksum >> 32) ^ checksum);
}
...
/* #ifdef FIXME */
  void
md_print_uop(enum md_opcode uop,
    md_inst_t instruction,	/* instruction to disassemble */
    md_addr_t pc,		/* addr of inst, used for PC-rels */
    FILE *stream)		/* output stream */
{
