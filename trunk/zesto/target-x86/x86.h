...
struct mem_t;
void md_fetch_inst(md_inst_t *inst, struct mem_t *mem, md_addr_t pc);
void md_use_fetched_inst(md_inst_t *inst, byte_t *mem, md_addr_t pc);
...
enum md_opcode md_decode(byte_t mode, md_inst_t *inst, int set_nop);
...
#if 0
#define BOFS        ((((dword_t)(inst & 0xffffff)) >> 23) ? ((dword_t)(-67108864 + (dword_t)(((inst & 0xffffff) << 2)))) : ((dword_t)((inst & 0xffffff)) << 2))
#endif
...
/* flags evaluator */
const int md_cc_eval(int cond, dword_t aflags, bool * bogus);
...
#define SEXT24(X)                            \
  (((X) & 0x800000) ? ((sdword_t)(X) | 0xff800000)) : (sqword_t)(X))

/* load/store opcode accessors */
#define LDST_PU        ((Mop->fetch.inst >> 23) & 0x03)
#define REGLIST        (Mop->fetch.inst & 0xffff)

/* floating point opcode accessors */
#define FPIMMBITS    (Mop->fetch.inst & 0x07)
#define FPIMM        (md_fpimm[FPIMMBITS])
extern double md_fpimm[8];

#define EF_SIZE        (((Mop->fetch.inst >> 18) & 0x02) | ((Mop->fetch.inst >> 7) & 0x01))
#define LDST_EF_SIZE    (((Mop->fetch.inst >> 21) & 0x02) | ((Mop->fetch.inst >> 15) & 0x01))
#define GH_RNDMODE    ((Mop->fetch.inst >> 5) & 0x03)
#define FCNT        (((Mop->fetch.inst >> 21) & 0x02) | ((Mop->fetch.inst >> 15) & 0x01))

/* ones counter */
#define ONES(EXPR)    (md_ones(EXPR))

/* ones counter function */
int md_ones(dword_t val);
...
int md_addc(dword_t lhs, dword_t rhs, dword_t res);
int md_addv(dword_t lhs, dword_t rhs, dword_t res);
int md_subc(dword_t lhs, dword_t rhs, dword_t res);
int md_subv(dword_t lhs, dword_t rhs, dword_t res);

/* SWI accessors */
#define SYSCODE        (Mop->fetch.inst & 0xffffff)

/* default target PC handling */
#ifndef SET_TPC
#define SET_TPC(PC)    Mop->decode.targetPC = (PC)
#define SET_TPC_W(PC)    Mop->decode.targetPC = (PC)
...
#define COND        (Mop->fetch.(inst >> 28) & 0x0f)

/* test instruction condition value */
#define COND_VALID(PSR)    (((COND) == COND_AL) || md_cond_ok(Mop->fetch.inst, PSR))

/* full capability instruction condition evaluator */
extern int md_cond_ok(md_inst_t inst, dword_t psr);

/* compute shifted register RM value */
#define SHIFTRM(RMVAL, RSVAL, CFVAL)                    \
  (((SHIFT_BITS)==0) ? (RMVAL) : md_shiftrm(Mop->fetch.inst, (RMVAL), (RSVAL), (CFVAL)))

/* full capability shifted register RM routine */
extern dword_t
md_shiftrm(md_inst_t inst, dword_t rmval, dword_t rs_val, dword_t psr);

#define SHIFTC(RMVAL, RSVAL, CFVAL)                    \
  (((SHIFT_BITS)==0) ? (CFVAL) : md_shiftc(Mop->fetch.inst, (RMVAL), (RSVAL), (CFVAL)))

/* full capability shifted register CF routine */
dword_t
md_shiftc(md_inst_t inst, dword_t rmval, dword_t rsval, dword_t cfval);
...
dword_t md_xor_regs(struct regs_t *regs);
...
#define MD_VALID_ADDR(ADDR) 1 // skumar - FIXME many addr were getting classified as invalid. possibly due to mmap implementation
...
word_t
md_uop_opc(enum md_opcode uopcode, struct Mop_t * Mop);

byte_t
md_uop_reg(enum md_xfield_t xval, struct Mop_t * Mop, bool * bogus);

byte_t
md_uop_immb(enum md_xfield_t xval, struct Mop_t * Mop, bool * bogus);

inline dword_t
md_uop_immv(enum md_xfield_t xval, struct Mop_t * Mop, bool * bogus);

inline dword_t
md_uop_lit(enum md_xfield_t xval, struct Mop_t * Mop, bool * bogus);

#ifdef ZESTO_ORACLE_C

#define XMEM_READ_BYTE(M,A)                        \
  ({ word_t _x; byte_t _mem_read_tmp;                               \
   _x = (word_t)MEM_READ_BYTE(M,A);                   \
   _x; })
...
#define XMEM_READ_DWORD(M,A)                        \
  ({ dword_t _x; byte_t _mem_read_tmp;                               \
   _x = ((dword_t)MEM_READ_BYTE(M,A)                    \
...
#define XMEM_READ_QWORD(M,A)                        \
  ({ qword_t _x; byte_t _mem_read_tmp;                           \
   _x = ((qword_t)MEM_READ_BYTE(M,A)                    \
