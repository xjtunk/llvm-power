...
#
if (@ARGV != 2) {
  die "Usage: extract_trace.pl <mode> <input_file>\n";
}
...
if ($ARGV[0] eq "-mase") {
...
elsif ($ARGV[0] eq "-macro") {
  $last_count = 1;
  $last_line = "";
  while (defined ($line = <INFILE>)) {
    if ($line =~ /spec/) { next; }
    ($first, $second, $third) = split(/:/, $line);
    ($junk, $count) = split(/\s+/, $second);
    $curr_line = "$count [xor:$third\n";
    if ($count != $last_count) { print $last_line; }
    $last_count = $count;
    $last_line = $curr_line;
  }
  print $last_line;
  ...
  close(INFILE);
  die "Invalid mode: use -mase or -macro\n";
}
