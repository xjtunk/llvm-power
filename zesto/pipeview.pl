...
#
# pipeview.pl - pipeline trace pretty printer
#
...
if (@ARGV != 1)
  {
    print STDERR "Usage: pipeview.pl <pipe_trace>\n";
    exit -1;
  }
...
while (<TRC_FILE>)
  {
	...
	foreach $pseq (keys %insts)
	  {
	    ...
	    if ($events & 0x00000020)		# checker error
	      {
		$evstr = $evstr . "?";
	      }
    ...
    elsif (/^\*\s+(\d+)\s+(\w+)\s+(0x[0-9a-fA-F]+)\s+(\d+)\s+(0x[0-9a-fA-F]+)\n?$/)
      {
	if ($insts{$1})
	  {
